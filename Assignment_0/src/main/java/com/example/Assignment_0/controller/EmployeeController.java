...
public class EmployeeController {
    ...
    public ResponseEntity<ApiResponse<Page<EmployeeAllocationDto>>> getAllEmployees(
        ...
        try {
            ...
            if (userDetails == null || userDetails.getAuthorities().stream().noneMatch(auth -> auth.getAuthority().equals("ROLE_ADMIN"))) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ApiResponse<>("Unauthorized access", 401, null));
            }
    ...
    @GetMapping("/{id}")
    private ResponseEntity<ApiResponse<EmployeeDto>> getEmployeeById(HttpServletRequest request, @PathVariable Long id){
        try {
            UserDetails userDetails = (UserDetails) request.getAttribute("userDetails");
            if (userDetails == null || userDetails.getAuthorities().stream().noneMatch(auth -> auth.getAuthority().equals("ROLE_ADMIN"))) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ApiResponse<>("Unauthorized access", 401, null));
            ...
            if (employee==null) return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponse<>("Employee with the given id: "+id+" not found" , 404, null));
            return ResponseEntity.ok(new ApiResponse<>("Retrieved Employee Successfully", 200, employee));
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(new ApiResponse<>(Arrays.toString(e.getStackTrace()), 500, null));
        }
    }

    @GetMapping("/bench")
    private ResponseEntity<ApiResponse<List<EmployeeDto>>> getBenchList(HttpServletRequest request){
        try {
            UserDetails userDetails = (UserDetails) request.getAttribute("userDetails");
            if (userDetails == null || userDetails.getAuthorities().stream().noneMatch(auth -> auth.getAuthority().equals("ROLE_ADMIN"))) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ApiResponse<>("Unauthorized access", 401, null));
            }
            return ResponseEntity.ok(new ApiResponse<>("Retrieved Bench List Successfully", 200, employeeService.getBenchList()));
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(new ApiResponse<>(Arrays.toString(e.getStackTrace()), 500, null));
        }
    }
    ...
    @PostMapping("/import")
    private ResponseEntity<ApiResponse<?>> importEmployees(HttpServletRequest request, @RequestBody MultipartFile file){
        try{
            UserDetails userDetails = (UserDetails) request.getAttribute("userDetails");
            if (userDetails == null || userDetails.getAuthorities().stream().noneMatch(auth -> auth.getAuthority().equals("ROLE_ADMIN"))) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ApiResponse<>("Unauthorized access", 401, null));
